...
function lessonTokens(query) {
  return [...new Set(String(query || '').toLowerCase().match(/[a-z0-9][a-z0-9_-]{2,}/g) || [])];
}

export function recallLessons(state, query = '', limit = MAX_RELEVANT_LESSONS) {
  const lessons = Array.isArray(state?.lessons) ? state.lessons : [];
  const tokens = lessonTokens(query);
  const scored = lessons.map((lesson, index) => {
    const haystack = `${lesson?.text || ''} ${lesson?.source || ''}`.toLowerCase();
    const score = tokens.reduce((total, token) => total + (haystack.includes(token) ? 1 : 0), 0);
    return { lesson, score, index };
  }).filter((item) => item.score > 0);
  scored.sort((a, b) => b.score - a.score || b.index - a.index);
  return scored.slice(0, Math.max(0, Math.min(MAX_RELEVANT_LESSONS, Number(limit) || MAX_RELEVANT_LESSONS))).map((item) => item.lesson);
}
...
async function cycleUnlocked(callbacks) {
  ...
  const lessonQuery = `${task.id} ${task.label} ${goal?.text || ''} ${goal?.criteria || ''}`;
  const lessons = recallLessons(state, lessonQuery);
  ...
  try {
    ...
    else { success = Boolean(await callbacks[task.id]({ goal, task, strategy: selected.name, lessons, strategyStats: selected, state })); status = success ? 'succeeded' : 'failed'; if (!success) lesson = `Strategy '${selected.name}' did not satisfy ${task.success}. Try another bounded strategy.`; }
  ...
  return { cycle: state.cycle, goal, task, strategy: selected.name, status, success, lesson, lessons, stats: state.strategies[selected.key], lessonCount: state.lessons.length, metrics: state.metrics };
}
...
export async function autonomyHealth() { const state = await loadAutonomyState(); const attempts = state.attempts.filter((item) => item.status !== 'skipped').length; return { ok: true, cycle: state.cycle, goals: state.goals.length, attempts, lessons: state.lessons.length, strategies: Object.keys(state.strategies).length, successRate: attempts ? (state.metrics.successes || 0) / attempts : 0, backend: 'file', version: state.version }; }

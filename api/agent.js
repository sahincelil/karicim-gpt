...
async function callModel(model, messages, apiKey, signal) {
...
async function runCustomTool(call) {
  ...
  if (!/^[A-Za-z0-9_.-]+$/.test(args.owner || '') || !/^[A-Za-z0-9_.-]+$/.test(args.repo || '') || typeof args.path !== 'string' || !args.path || args.path.length > 300) {
    throw new Error('GitHub parametreleri geçersiz.');
  }
...
async function runModelLoop(model, userMessages, apiKey, signal) {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
    ...
    if (!response.ok) {
      ...
      error.retryable = response.status === 400 || response.status === 404 || response.status === 408 || response.status === 429 || response.status >= 500;
      throw error;
    }
    ...
    const unsupported = localCalls.filter((call) => call?.function?.name !== 'github_read_public_file');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (controller.signal.aborted) return send(res, 504, { error: 'Agent zaman aşımına uğradı.' });
    if (lastError?.message === 'agent:tool-budget') return send(res, 502, { error: 'Agent araç kullanım sınırına ulaştı.' });
    if (lastError?.message === 'agent:round-budget') return send(res, 502, { error: 'Agent işlem adımı sınırına ulaştı.' });
    return send(res, 502, { error: 'Agent çalıştırılamadı. Ücretsiz model veya araç geçici olarak kullanılamıyor.' });
  } catch (error) {
    console.error('Agent error:', { name: error?.name, message: error?.message });
    return send(res, error?.name === 'AbortError' || controller.signal.aborted ? 504 : 502, { error: error?.name === 'AbortError' || controller.signal.aborted ? 'Agent zaman aşımına uğradı.' : 'Agent çalıştırılamadı.' });
  } finally {
